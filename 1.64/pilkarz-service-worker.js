const CACHE_NAME='polski-pilkarz-current-1.64';
const APP_SHELL=[
  './index.html','./pilkarz.webmanifest','./polska-kariera.css',
  ...
  './pilkarz-icons/icon.svg','./pilkarz-icons/icon-192.png','./pilkarz-icons/icon-512.png'
];
...
self.addEventListener('activate',event=>{
  event.waitUntil(caches.keys().then(keys=>Promise.all(
    keys.filter(key=>key.startsWith('polski-pilkarz-current-')&&key!==CACHE_NAME).map(key=>caches.delete(key))
  )));
  self.clients.claim();
});
